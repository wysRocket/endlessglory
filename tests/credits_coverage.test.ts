...
const PACK_COVERED_DIRS: Record<string, string> = {
  ...
  // Keys are path prefixes, not just top-level directories, because the
  // Emberwood tree mixes both kinds: its buildings, foliage and weapons are bulk
  // re-exports of packs credited as a whole, while its NPCs and creature are
  // individually sourced and must be named file by file.
  'emberwood/eastbrook': 'Emberwood Eastbrook buildings and props',
  'emberwood/foliage': 'Emberwood foliage',
  'emberwood/weapons': 'Emberwood weapons',
};

/** Directories where every file must be named individually in CREDITS.md. */
const PER_FILE_DIRS = ['creatures', 'chars', 'kawaii', 'emberwood/npcs', 'emberwood/creatures'];
...
describe('CREDITS.md coverage', () => {
  ...
  it('covers every remaining model by a pack row', () => {
    const orphans = allGlb
      .filter((f) => !PER_FILE_DIRS.some((d) => f.startsWith(`${d}/`)))
      .filter((f) => {
        const covered = Object.keys(PACK_COVERED_DIRS).some((d) => f.startsWith(`${d}/`));
        return !covered && !credited.has(path.basename(f));
      });
    expect(
      orphans,
  ...
  it('keeps the unresolved-provenance section, and every asset it names still ships', () => {
    ...
    for (const file of [
      ...
      'CombatMech.glb',
      'skeleton_dagger.glb',
      'emberwood_armorer.glb',
      'emberwood_provisioner.glb',
    ]) {
      expect(
