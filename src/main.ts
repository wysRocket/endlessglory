...
import { isOfflineModeAvailable, playCtaTarget } from './game/offline_mode_gate';
...
function wireStartScreens(): void {
  ...
  // Every offline / dropdown hook below resolves defensively: a partial DOM
  // fixture or an embedded shell can omit the #btn-offline compat trigger, the
  // #offline-select panel, or the realm dropdown, and wiring is skipped when the
  // markup is absent. When offline mode is unavailable for this build
  // (offlineAvailable false) the trigger is left unwired too, so no caller
  // (including a stale E2E script) can reach it.
  if (offlineBtn) {
    if (offlineAvailable) {
  ...
  if (serverSelect && serverTrigger && serverMenu && btnPlay) {
    ...
    // This landing page renders no realm dropdown, so Play is the only entry.
    // Route on whether offline is actually AVAILABLE, never on whether
    // #btn-offline exists: that element is a hidden legacy automation hook (see
    // scripts/enter_offline_game.mjs) and is present in production too, so
    // keying off it sent the production Play button into handleOfflineSelect,
    // which early-returns on !offlineAvailable and left the CTA dead.
    const playTarget = playCtaTarget({
      hasOfflineHook: Boolean(offlineBtn),
      offlineAvailable,
    });
    btnPlay.addEventListener(
      'click',
      playTarget === 'offline' ? handleOfflineSelect : handleOnlineSelect,
    );
  }
