...
// ---------------------------------------------------------------------------
// Stylized splat layers, generated by src/render/terrain_texture_core.ts and
// written to public/textures/terrain by `npm run terrain:textures`. They replace
// the ambientCG photo scans this used to sample: photoreal ground was the last
// loud mismatch under a hand-painted cast. The scans stay on disk because the
// voxel_terrain prototype still samples them.
...
// ~5MB of PNGs, skipped when the URL already forces the Lambert tier (an
// auto-detected low tier still fetches them; the URL guess can't know yet)
if (GFX.terrainSplat) {
  kickTerrainTex('grassC', 'StylizedGrass_Color.png', true);
  kickTerrainTex('grassN', 'StylizedGrass_NormalGL.png', false);
  kickTerrainTex('dirtC', 'StylizedDirt_Color.png', true);
  kickTerrainTex('dirtN', 'StylizedDirt_NormalGL.png', false);
  kickTerrainTex('rockC', 'StylizedRock_Color.png', true);
  kickTerrainTex('rockN', 'StylizedRock_NormalGL.png', false);
  kickTerrainTex('sandC', 'StylizedSand_Color.png', true);
  kickTerrainTex('sandN', 'StylizedSand_NormalGL.png', false);
  kickTerrainTex('mudC', 'StylizedMud_Color.png', true); // marsh wet mud (dirt variant)
  kickTerrainTex('snowC', 'StylizedSnow_Color.png', true);
}
  ...
  mat.onBeforeCompile = (sh) => {
    ...
    sh.fragmentShader = sh.fragmentShader
      ...
      .replace(
        ...
        // The stylized grass tiles exactly and already carries its own macro
        // scale, so the old two-scale self-blend is gone: mixing two posterized
        // samples multiplied the band count back up and softened the flat
        // hand-painted look the layer exists to produce.
        vec3 grassAlb = texture2D(uGrass, tuv).rgb;
