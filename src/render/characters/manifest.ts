...
export const VISUALS: Record<string, VisualDef> = {
  ...
  // Halloween creature (Meshy-generated) auto-rigged to the shared humanoid
  // skeleton via scripts/rig_kawaii_char.py; walk/attack graft from the warrior
  // clip donors (KAWAII_ANIM_URLS) plus the baked bind-pose breathing idle. Wired
  // as the forest wolf's replacement via MOB_KEYS.
  mob_hallow: {
...
const MOB_KEYS: Record<string, string> = {
  ...
  wild_boar: 'mob_boar',
  // The common forest wolf now wears the Halloween creature model (the other
  // beasts keep the wolf via FAMILY_KEYS.beast).
